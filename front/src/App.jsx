...
export default function App() {
  ...
  const navigate = (nextView) => {
    ...
    if (!isAuthenticated && privateViews.includes(nextView)) {
      setAuthReason('Ingresa a tu cuenta para ver esta sección de forma segura.');
      setView('login');
  ...
  return (
    <div className="app-shell">
      ...
      <main className="page-shell">
        ...
        {view === 'checkout' && (
          <CheckoutPage event={currentSelectedEvent} onBack={() => navigate('detail')} />
        )}
        {view === 'profile' && <ProfilePage />}
        {view === 'history' && <HistoryPage />}
        {view === 'favorites' && <FavoritesPage events={events} onOpenEvent={openEvent} />}
        {view === 'pqrs' && <PqrsPage />}
        {view === 'login' && <LoginPage reason={authReason} />}
      </main>
    </div>
  );
}
