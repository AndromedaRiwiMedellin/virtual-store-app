...
function toAbsoluteImage(posterUrl, title = '') {
  if (posterUrl?.startsWith('http')) return posterUrl;
  if (posterUrl) return `${MEDIA_BASE_URL}/${posterUrl.replace(/^\/+/, '')}`;

  const seed = encodeURIComponent(title || 'orbix-event');
  return `https://source.unsplash.com/1200x800/?event,${seed}`;
}
...
export function normalizeEvent(payload, index = 0) {
  ...
  return {
    ...
    image: toAbsoluteImage(event.posterUrl, event.title),
    date,
...
export function getEvents(filters = {}) {
  ...
  return request(`/events${params.toString() ? `?${params}` : ''}`)
    .then((events) => events.map((event, index) => normalizeEvent(event, index)));
}
